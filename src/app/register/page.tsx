...
const Button = ({ children, variant = "default", disabled = false, onClick, type = "button" }) => {
  const baseStyles = "inline-flex items-center justify-center rounded-lg font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:pointer-events-none disabled:opacity-50";
  const variants = {
...
export default function RegisterPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    totalMembers: 2,
    members: [
      { name: '', designation: '', department: '', city: '', state: '', mobile: '', email: '' },
      { name: '', designation: '', department: '', city: '', state: '', mobile: '', email: '' },
    ],
    problemDomain: '',
    projectTitle: '',
    ...
    if (targetLength !== currentLength) {
      if (targetLength > currentLength) {
        ...
        for (let i = currentLength; i < targetLength; i++) {
          newMembers.push({ name: '', designation: '', department: '', city: '', state: '', mobile: '', email: '' });
        }
        setFormData(prev => ({ ...prev, members: newMembers }));
      } else if (targetLength < currentLength) {
        setFormData(prev => ({ ...prev, members: prev.members.slice(0, targetLength) }));
      }
    }
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: type === 'checkbox' ? checked : (name === 'totalMembers' ? parseInt(value) : value)
    }));
  };
  ...
  const validateForm = () => {
    ...
    formData.members.forEach((member, index) => {
      ...
      if (!member.designation) newErrors[`member${index}Designation`] = 'Designation is required';
      ...
      if (!member.mobile) newErrors[`member${index}Mobile`] = 'Mobile is required';
      if (!member.email || !/\S+@\S+\.\S+/.test(member.email)) {
        newErrors[`member${index}Email`] = 'Valid email is required';
      }
    });
    
    if (!formData.bonafideFile) newErrors.bonafideFile = 'Bonafide certificate is required';
    ...
    if (!formData.githubRepoLink) newErrors.githubRepoLink = 'GitHub link is required';
    if (!formData.demoVideoURL) newErrors.demoVideoURL = 'Demo video URL is required';
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-left">
                            ...
                            <p className="text-xs text-gray-400">
                              {(formData.bonafideFile.size / 1024).toFixed(2)} KB
                            </p>
                          </div>
                        </div>
                        <label
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  >
                    ...
                    understand that my team must have a working PoC with TRL 3 or
                    above and that all submissions must be original work.
                  </label>
                </div>
                {errors.agreeToRules && (
